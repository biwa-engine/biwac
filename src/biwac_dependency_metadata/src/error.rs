    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::UnexpectedEnd { needed, available } => {
                write!(
                    f,
                    "unexpected end of data: needed {needed} bytes, got {available}"
                )
            }
            ...
            Self::StringOffsetOutOfBounds { offset, table_len } => {
                write!(
                    f,
                    "string offset {offset} out of bounds (table len: {table_len})"
                )
            }
            ...
            Self::SymbolIndexOutOfBounds { index, sym_count } => {
                write!(
                    f,
                    "symbol index {index} out of bounds (sym count: {sym_count})"
                )
            }
            Self::FileIndexOutOfBounds { index, file_count } => {
                write!(
                    f,
                    "file index {index} out of bounds (file count: {file_count})"
                )
            }
