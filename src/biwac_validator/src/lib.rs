...
use biwac_name_resolver::{ModSym, PkgSymMap};
...
type VldResult<T> = Result<T, ValidateError>;

#[derive(Debug)]
pub struct Pkg {}

impl Pkg {
    pub fn try_validate(_pkg: PkgSymMap) -> VldResult<Self> {
        todo!()
    }
}
...
//
//             if let Some(ModSym::VarDec(g)) = self.pkg.syms().get(&global_var_absid) {
//                 Some(Variable {
//                     typ: g.typ.clone(),
...
//         if let Some(ModSym::FnDef(f)) = self.pkg.syms().get(absid) {
//             Some(f)
//         } else {
//             None
//         }
...
//         if let Some(ModSym::TypeDef(TypeDefContent::Struct(ssign))) = self.pkg.syms().get(absid) {
//             Some(ssign)
//         } else {
//             None
//         }
